...
CAPTCHA_API = "https://api.amusingthrone.com/wizard101/v1/captcha"
...
def get_solvable_trivias(trivia_page):
	...
	for trivia in trivias_elements:
		if trivia.get("class")[0] != "notake":
			...
			log("Skipping \"{}\", already taken.".format(
                            trivia.find("div", {"class": "gamename"}).text), "error")
...
def solve_trivias(trivias, trivia_data):
	...
	for trivia in trivias:
		if trivia[0] in trivia_data.keys():
			...
			tform = soup.find("div", {"id": "quizFormComponent"}).find(
                            'input', {'name': 't:formdata'}).get('value').strip()
			...
			for x in range(12):
				...
				question_id = soup.find(
                                    'input', {'name': 'questionId'}).get('value').strip()
				...
				if question in trivia_answers:
					if trivia_answers[question] in answers:
						...
						answer_id = answer_id.find(
                                                    'input', {'name': 'answers'}).get('value').strip()
					else:
				...
				connection.cookies.set("fkigvideo", quiz_id,
				                       domain="www.freekigames.com", path='/')
				question = connection.post(
                                    FREEKIGAMES_QUIZFORM, data=question_data, headers=headers)
				soup = question.text
...
def post_captcha(word):
	post = connection.post(CAPTCHA_POST_URL, data={
	                       'value': word}, headers=headers)
	if post.text == 'true':
		return True
	return False
	...
	while not solved:
		if get_captcha() == True:
			response = requests.post(
                            CAPTCHA_API, files={'img': (FILENAME, open(FILENAME, 'rb'))})
...
def get_crowns(word, path):
	connection.cookies.set("showRegister", "block",
	                       domain="www.freekigames.com", path='/')
